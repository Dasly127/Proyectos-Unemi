...
class PILAS_NUM:
    ...
    def intercalar_pilas(self):
        try:
            self.pila_intercalada = self._intercalar(self.pila1, self.pila2)
    ...
    def _intercalar(self, pila1, pila2):
        intercalada = []
        pila_mayor, pila_menor = (pila2, pila1) if len(pila2) > len(pila1) else (pila1, pila2)
        
        for i in range(len(pila_menor)):
            intercalada.append(pila_mayor[i])
            intercalada.append(pila_menor[i])
        
        intercalada.extend(pila_mayor[len(pila_menor):])
        return intercalada

    def sumar_pilas(self):
        try:
            self.pila_suma = self._sumar(self.pila1, self.pila2)
            print("Pila 1 original:", self.pila1)
            print("Pila 2 original:", self.pila2)
            print("Suma ajustada de las pilas:", self.pila_suma)
        except Exception as e:
            print(f"Error al sumar pilas: {e}")

    def _sumar(self, pila1, pila2):
        min_len = min(len(pila1), len(pila2))
        suma = [pila1[i] + pila2[i] for i in range(min_len)]
        
        if len(pila1) > min_len:
            suma.extend(pila1[min_len:])
        else:
            suma.extend(pila2[min_len:])
